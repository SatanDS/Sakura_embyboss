...
提交地址后，Bot 会在聊天底部回复本条消息，显示新增/删除数量、已存在数量及当前总数，并同步更新原菜单。输入错误、保存失败、取消或超时也会发送结果消息；重复添加只提示已存在，不会重复保存。
...
当前 DuSheng 部署的 Bot 主面板提供「💵購買續期註冊碼」按钮，指向 `https://xf.dusheng.lol/payments/shop`，新用户和已有账号用户都可以打开。使用其他销售域名的部署需同步修改 `bot/func_helper/fix_bottons.py` 中该按钮 URL；`/pay` 命令的地址仍取自 `payments.public_url`。按钮只打开购买网站，不在 Telegram 内创建 Stripe 结账，实际下单仍受网站登录、购买须知及销售开关限制。
...
同一数据库的商品、注册席位和账号权益并非各环境独立。当前订单列表默认按支付环境分开，复用待支付订单和计算商品销售上限也按环境分别处理；归档不会免除同环境订单的销售占用。测试记录、注册预留和已兑换权益不会随切换自动清空，这也是完整验收推荐隔离部署的原因。
...
「订单管理」和「我的订单」默认显示当前支付环境的未归档订单。正式模式下不再将测试订单混入列表；后台「正式已支付金额（列表）」只合计当前列表中的正式已支付订单，不计测试款，最多对应本次加载的 500 笔记录，不是 Stripe 的实际结算余额。

清理测试验收留下的记录时，所有者登录「销售管理 → 订单管理」，将支付环境选为「全部环境」、归档状态选为「未归档」，勾选需要整理的订单，点击「归档所选」。核对确认框中的订单号和测试/正式数量，勾选确认后提交，每次最多 100 笔。在「已归档」中仍可查看原记录、勾选并「恢复所选」。普通管理员可以查询，只有所有者有归档/恢复权限。

可归档的记录为测试订单，以及尚未收款/发码、没有退款、争议或待核查标记的正式待支付/已过期订单。正式已支付、已发码或存在财务异常的订单禁止归档。若提交期间订单状态已改变，后端重新校验整个批次，不会只归档其中一部分。

**归档仅整理列表，不删除数据，不取消 Stripe 付款，也不停止后台交付。** 兑换码、任务、事件、注册席位、套餐价格和已有权益均保留；已兑换的测试码不会因为归档而扣回账号时长。正式未支付订单在归档后若被 Stripe 确认付款或产生新的待核查事项，会重新显示，仍按原流程发码。`has_checkout=0` 仅说明没有本地 Stripe 会话记录，不足以证明远端从未创建付款对象。
